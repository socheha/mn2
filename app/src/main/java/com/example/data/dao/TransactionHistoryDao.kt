...
interface TransactionHistoryDao {
    @Query("SELECT * FROM transaction_history_logs ORDER BY timestamp DESC, id DESC")
    fun getAllLogs(): Flow<List<TransactionHistoryLogEntity>>

    @Query("SELECT * FROM transaction_history_logs ORDER BY timestamp DESC, id DESC")
    suspend fun getAllLogsList(): List<TransactionHistoryLogEntity>

    @Query("SELECT * FROM transaction_history_logs WHERE transactionType = :type AND transactionId = :txId ORDER BY timestamp DESC")
    fun getLogsForTransaction(type: String, txId: Long): Flow<List<TransactionHistoryLogEntity>>

    @Query("SELECT * FROM transaction_history_logs WHERE tanggal >= :startDate AND tanggal <= :endDate ORDER BY timestamp DESC, id DESC")
